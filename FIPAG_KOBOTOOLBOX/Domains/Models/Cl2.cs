using System;
using System.Collections.Generic;

namespace FIPAG_KOBOTOOLBOX.Domains.Models
{
    public partial class Cl2
    {
        public string Cl2stamp { get; set; } = null!;
        public string Codpais { get; set; } = null!;
        public string Descpais { get; set; } = null!;
        public bool Adcsepaativa { get; set; }
        public bool Isb2b { get; set; }
        public bool Monitignios { get; set; }
        public string Nomemod { get; set; } = null!;
        public decimal Nomod { get; set; }
        public decimal Estabmod { get; set; }
        public string Cadmintipo1 { get; set; } = null!;
        public string Cadmintipo1stamp { get; set; } = null!;
        public string Cadmintipo2 { get; set; } = null!;
        public string Cadmintipo2stamp { get; set; } = null!;
        public string Cadmintipo3 { get; set; } = null!;
        public string Cadmintipo3stamp { get; set; } = null!;
        public string Cadmintipo4 { get; set; } = null!;
        public string Cadmintipo4stamp { get; set; } = null!;
        public bool Clivacaixa { get; set; }
        public string Pwportal { get; set; } = null!;
        public string Userid { get; set; } = null!;
        public DateTime Dlogin { get; set; }
        public string Hlogin { get; set; } = null!;
        public string Tdocidcod { get; set; } = null!;
        public bool Tdocidenif { get; set; }
        public string Retarrat { get; set; } = null!;
        public bool Dgrelhas { get; set; }
        public bool Dbpm { get; set; }
        public bool Doceletronicos { get; set; }
        public decimal Latitude { get; set; }
        public decimal Longitude { get; set; }
        public string Ubigeo { get; set; } = null!;
        public bool Cobrecsede { get; set; }
        public string Forgotid { get; set; } = null!;
        public DateTime Forgotdate { get; set; }
        public bool Termsconditions { get; set; }
        public string UAndar { get; set; } = null!;
        public string UEndereco { get; set; } = null!;
        public string UNrua { get; set; } = null!;
        public string UPorta { get; set; } = null!;
        public string UPref { get; set; } = null!;
        public string URua { get; set; } = null!;
        public string Ousrinis { get; set; } = null!;
        public DateTime Ousrdata { get; set; }
        public string Ousrhora { get; set; } = null!;
        public string Usrinis { get; set; } = null!;
        public DateTime Usrdata { get; set; }
        public string Usrhora { get; set; } = null!;
        public bool Marcada { get; set; }
        public bool UEstabon { get; set; }
        public decimal UAnterior { get; set; }
        public decimal UConsmin { get; set; }
        public string UDiametro { get; set; } = null!;
        public bool UEmpreit { get; set; }
        public bool UEndsede { get; set; }
        public DateTime UInicio { get; set; }
        public decimal UMcubico { get; set; }
        public decimal UMcubicob { get; set; }
        public string UMemo { get; set; } = null!;
        public string UNacordo { get; set; } = null!;
        public string UNcontado { get; set; } = null!;
        public decimal UOrdem { get; set; }
        public string UProcdesc { get; set; } = null!;
        public decimal UProcesso { get; set; }
        public string URefcon { get; set; } = null!;
        public DateTime UTermino { get; set; }
        public string UDimramal { get; set; } = null!;
        public decimal UDist { get; set; }
        public string UCalramal { get; set; } = null!;
        public string UGpsx { get; set; } = null!;
        public string UGpsy { get; set; } = null!;
        public string Egarprod { get; set; } = null!;
        public string Egarapa { get; set; } = null!;
        public string Egarpgl { get; set; } = null!;
        public string Egargrupo { get; set; } = null!;
        public string Egaropera { get; set; } = null!;
        public string Egaropdes { get; set; } = null!;
        public string Validadecartao { get; set; } = null!;
        public string Trackfuelpos { get; set; } = null!;
        public string Passpais { get; set; } = null!;
        public string Passpaisdesc { get; set; } = null!;
        public bool Refmbusavalidade { get; set; }
        public decimal Refmbtpdata { get; set; }
        public decimal Refmbndias { get; set; }
        public decimal Cativaperc { get; set; }
        public bool Nifvalidado { get; set; }
        public DateTime UDtprepg { get; set; }
        public decimal UNcapita { get; set; }
        public string UClientid { get; set; } = null!;
        public DateTime UIniciof { get; set; }
        public DateTime UDtactgps { get; set; }
        public string UMarcco { get; set; } = null!;
        public string UModeco { get; set; } = null!;
        public string ULacre { get; set; } = null!;
        public DateTime UDtultact { get; set; }
        public decimal UPercdesv { get; set; }
        public decimal UOtpapp { get; set; }
        public decimal UMmedia { get; set; }
        public string URefpag { get; set; } = null!;
        public bool UClprepg { get; set; }
        public string Codendereco { get; set; } = null!;
        public bool UAlugact { get; set; }
        public bool UTemtxdsp { get; set; }
        public bool UTemtxsan { get; set; }
        public string UNcasa { get; set; } = null!;
        public string UNtalhao { get; set; } = null!;
        public string UNquart { get; set; } = null!;
        public string UDistrit { get; set; } = null!;
        public string UProvince { get; set; } = null!;
        public decimal UDesvio { get; set; }
        public decimal UConsuman { get; set; }
        public bool UTemesc0 { get; set; }
    }
}
